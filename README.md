...
| `notes` | `notes-collector` | Event-package baseline for Obsidian/Notion plus authorized Youdao/Evernote/Markdown/HTML/JSON/ENEX/ZIP imports; manifest reports P1 note-platform coverage, per-input source audit, skip reasons, and missing platforms |
...
| `social-activity` | `social-activity` | Local JSON/CSV/Excel/HTML/TXT/ZIP activity baseline for Weibo/Bilibili/Xiaohongshu follows, likes, favorites, watch history, comments, shares, creators, engagement counts, weak influence signals, and platform/action/field coverage manifests |
...
| `china-wealth-assets` | `china-wealth-assets` | Normalized local export/package baseline for fund, wealth, cash-management holdings, transactions, asset snapshots, Excel/JSON/CSV/ZIP inputs, source audit, platform/field coverage, asset value summary, and SoulMirror lake sync |
| `financial-news-usage` | `financial-news-usage` | Local usage export, saved-page, ZIP, and browser-history baseline for finance-app read, favorite, subscribe, search, and alert actions with platform/action coverage, per-input source audit, skip reasons, and path-safety manifest |
| `hk-us-brokerage` | `hk-us-brokerage` | Local read-only CSV/JSON/Excel/ZIP export baseline for Futu/Tiger/IBKR assets, positions, executions, orders, cashflows, dividends, FX, margin, tax, settlement fields, broker/trade-surface/field coverage, per-input source audit, skip reasons, and path-safety manifest |
| `pro-terminal-usage` | `pro-terminal-usage` | Local workflow CSV/JSON/Excel/HTML/TXT/LOG/ZIP export baseline for Wind/Choice/iFinD/Bloomberg workspaces, watchlists, searches, downloads, templates, factors, datasets, fields, function codes, terminal/activity/field coverage, per-input source audit, skip reasons, and license-safe manifest |
