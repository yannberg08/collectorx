...
| `wechat` | `wechat-export` | WeChat 4.x local query baseline with `--collect --out-dir` CollectorX package output, validator-safe preflight/no-message data-quality gap packages, message/gap/usable counts, field/filter/source audit manifest, and generic-to-lens boundary; real-source validation still requires authorized keys/platform setup |
