...
| --- | --- | --- |
| `filesystem` | `filesystem-collector` | Metadata-only generic file collector aligned with SoulMirror driver boundary; manifest includes source audit, extension/path/file-name/directory/keyword scope-policy audit, validator-safe data-quality gap packages, usable/filesystem/file/gap counts, filesystem/data-quality/research-documents-lens readiness gates, filesystem boundary proof, skip reasons, per-root results, and macOS/Windows/Linux default-root plan; investor routing is handled by lenses |
| `wechat` | `wechat-export` | WeChat 4.x local query baseline with `--diagnose` local-source readiness JSON, `--collect --out-dir` CollectorX package output, validator-safe preflight/no-message data-quality gap packages, message/gap/usable counts, field/filter/source audit manifest, and generic-to-lens boundary; real-source validation still requires authorized readable DB/key/platform setup |
| `feishu` | `feishu` | Legacy OAuth/API tool plus authorized local/ZIP export package baseline for Feishu messages, documents, file refs, folders, meetings, recordings, source audit, ZIP skip reasons, path-level parse results, validator-safe missing/no-readable data-quality gaps, usable/feishu/gap counts, and Feishu/data-quality/downstream-lens readiness gates; real account API validation pending |
...
| `email` | `email-collector` | Privacy-preserving `preflight --diagnose` readiness JSON plus multi-account IMAP, authorized local email scan, and EML/EMLX/Maildir/MBOX/JSON/CSV/ZIP package output with account/folder audit, local-scan/import audit, skipped file/ZIP-member reasons, email authorization scope-policy audit, validator-safe filtered-all/no-input gap packages, email/gap event counts, email-lake/data-quality/email-research readiness gates, field coverage, body/attachment policy, and generic-to-lens boundary; real mailbox/root validation still pending |
...
| `wechat-favorites` | `wechat-favorites` | Local saved-article/favorites/read/share file/folder/ZIP baseline with action coverage, source-account/action/tag/domain/keyword scope-policy audit, validator-safe missing/no-readable/filtered-all data-quality gaps, usable/wechat-favorite/favorite/gap counts, WeChat-favorites/data-quality/article-lens readiness gates, per-input source audit, skip reasons, and path-safety manifest; real WeChat favorites adapter pending |
| `social-activity` | `social-activity` | Local JSON/CSV/Excel/HTML/TXT/ZIP/browser-history activity baseline for Weibo/Bilibili/Xiaohongshu follows, likes, favorites, watch history, comments, shares, browser-history visits, creators, engagement counts, visit/typed counts, browser transition types, social influence topics, weak influence signals, fixed social-activity-to-social-investment-influence E2E fixture gate, platform/action/source-app/domain/creator/topic/keyword scope-policy audit, validator-safe filtered-all/no-input data-quality gap packages, gap/usable/social-activity counts, explicit social-activity/data-quality/social-investment-lens readiness gates, platform/action/field coverage, social activity boundary proof, per-input source audit, skipped reasons, and ZIP member audit |
...
| `eastmoney-portfolio` | `eastmoney-portfolio` | macOS local + authorized full trade Lake production candidate with event-kind/symbol/account/source/keyword authorization scope-policy audit, validator-safe filtered-all gap packages, gap/usable/strong-trade counts, Investor Wiki evidence that excludes gap events from business facts, and strong trade boundary proof; strong trade tables require unlocked trading account |
| `xueqiu-watchlist` | `xueqiu-watchlist` | Authorized local export/package baseline for Xueqiu watchlists with source audit, symbol/market/group/industry/tag/keyword scope-policy audit, validator-safe filtered-all/no-input gap packages routed to data-quality lake, gap/usable/watchlist counts, explicit business/data-quality/Wiki readiness gates, field coverage, watchlist boundary proof, and 7/20 Wiki evidence package with raw/gap/usable generated_from observability; attention-universe evidence only |
| `xueqiu-investor-activity` | `xueqiu-investor-activity` | Safe `diagnose` preflight plus normalized local export/package/HAR/browser-history baseline for watchlists, follows, posts, comments, favorites, saved pages, owner portfolios, browser-history visits, source apps, visit/typed counts, transition types, domain-filter audit, source/pagination audit, fixed offline activity fixture gate, activity/source-surface/source-app/domain/symbol/author/keyword scope-policy audit, local-path/token/phone redaction, validator-safe filtered-all/no-input/preflight gap packages routed to data-quality lake, gap/usable/activity counts, explicit business/data-quality/Wiki readiness gates, field coverage, Investor Wiki evidence raw/gap/usable generated_from observability, and SoulMirror lake sync; real account adapter pending |
| `china-wealth-assets` | `china-wealth-assets` | Normalized local export/package baseline for fund, wealth, cash-management holdings, transactions, asset snapshots, Excel/legacy-XLS/HTML-table/PDF-statement/JSON/CSV/ZIP/HAR inputs, source preflight diagnostics, minimum asset-fact field-contract gates, platform/account/subtype/product/currency/side/keyword scope-policy audit, validator-safe no-input/unsupported-source/filtered-all/field-contract gap packages routed to data-quality lake, gap/usable/asset counts, explicit business/data-quality/Wiki readiness gates, source/PDF/HAR audit, platform/field/account/asset-surface/currency coverage, asset value summary, asset-boundary proof, and SoulMirror lake sync |
| `financial-news-usage` | `financial-news-usage` | Local usage export, saved-page, ZIP, direct Chromium/Safari browser-history, and ZIP-packaged browser-history baseline for finance-app read, favorite, subscribe, search, and alert actions with platform/action/topic coverage, behavior summary, platform/action/source-app/domain/topic/keyword scope-policy audit, filtered-all/no-input data-quality gap packages, gap/usable/usage counts, explicit business/data-quality/Wiki readiness gates, browser source counts, per-input source audit, skip reasons, and path-safety manifest |
| `hk-us-brokerage` | `hk-us-brokerage` | Local read-only CSV/JSON/Excel/ZIP export baseline for Futu/Tiger/IBKR assets, positions, executions, orders, cashflows, dividends, FX, margin, tax, settlement fields, account-boundary/currency-market/fee-tax-margin summaries, broker/account/subtype/symbol/market/currency/keyword scope-policy audit, validator-safe filtered-all/no-input data-quality gap packages, gap/usable/brokerage/strong-trade counts, explicit business/data-quality/Wiki readiness gates, unified brokerage-boundary proof, broker/trade-surface/field coverage, per-input source audit, skip reasons, and path-safety manifest |
| `pro-terminal-usage` | `pro-terminal-usage` | Local workflow CSV/JSON/Excel/HTML/TXT/LOG/ZIP export baseline for Wind/Choice/iFinD/Bloomberg workspaces, watchlists, searches, downloads, templates, factors, datasets, fields, function codes, query terms, parameters, export paths, row counts, workspace/template IDs, object counts, terminal/activity/field/topic coverage, workflow-intensity summary, terminal/activity/workspace/project/dataset/field/keyword scope-policy audit, validator-safe filtered-all/no-input data-quality gap packages, gap/usable/workflow counts, explicit business/data-quality/Wiki readiness gates, unified workflow-boundary proof, per-input source audit, skip reasons, and license-safe manifest |
...
| `research-documents` | `filesystem`, `notes` | Runnable lens baseline for research files and valuation docs; explicit XLS/XLSX/PPTX/DOCX/PDF content extraction, upstream filesystem-event Windows/Linux/macOS path-style and source-platform boundary summaries, extension/path/file-name/parser/surface/keyword scope-policy audit, validator-safe no-input/no-readable/no-match/filtered-all data-quality gap packages, required research/gap/usable manifest counts, path-level collection audit, skipped reasons, limit truncation, and screenshot metadata-only/no-OCR policy validation done |
| `email-research` | `email` | Runnable lens baseline for broker research, morning meetings, roadshow invites, IR mail, earnings alerts, research attachment refs, sender-domain/body-preview/attachment boundaries, sender-domain/folder/mailbox/subject/attachment/surface/keyword scope-policy audit, validator-safe profile gap packages routed to data-quality collection gaps, email-research/gap/usable manifest counts, and email-research boundary proof; real mailbox validation pending |
...
| `investment-notes` | `notes` | Runnable lens baseline for investment notes, reviews, rules, checklists, valuation assumptions, source-app surface, and preview/full-content surface; fixed E2E fixture proves generic notes do not write Wiki facts directly, the lens filters out non-investment notes, and generic notes gap packages stay in data-quality routing |
...
| `social-investment-influence` | `social-activity` | Runnable lens baseline for weak investment creator, social influence topic, fixed upstream social-activity fixture E2E gate, platform/action/source-app/domain/creator/topic/keyword scope-policy audit, filtered-all data-quality gap state, social-influence/data-quality/Wiki readiness gates, corroboration-candidate signals, and social influence boundary proof |
