...
| `research-documents` | `filesystem`, `notes` | Runnable lens baseline for research files and valuation docs; explicit XLS/XLSX/PPTX/DOCX/PDF content extraction, extension/path/file-name/parser/surface/keyword scope-policy audit, validator-safe no-input/no-readable/no-match/filtered-all data-quality gap packages, research/gap/usable manifest counts, path-level collection audit, skipped reasons, limit truncation, and screenshot metadata-only/no-OCR policy validation done |
