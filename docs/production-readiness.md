...
`calendar` and `task-calendar-investor` now carry stronger time-quality evidence:
...
- `task-calendar-investor` lens surface summaries now preserve duration,
  multi-day, and invalid-time counts so FinClaw can inspect planning evidence
