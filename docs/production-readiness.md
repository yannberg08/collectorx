...
The latest completed parallel wave hardens three P0 investor-avatar sources
before the next real-account validation push:

- `email-collector` is upgraded to `0.5.9` with privacy-preserving mailbox
  readiness diagnosis.
- `xueqiu-investor-activity` is upgraded to `0.3.8` with safe local-source
  preflight diagnosis for authorized exports, HAR files, copied browser
  history files, and copy-required browser-history candidates.
- `china-wealth-assets` is upgraded to `0.4.9` with source preflight
  diagnostics and `china_wealth_asset_fact_contract.v1` minimum field gates so
  placeholder rows cannot become asset facts.

For `email`:

- `email_api.py preflight --diagnose` now emits
  `collectorx.email_preflight.v1` JSON before any IMAP login, message header,
  message body, attachment, or local email file content is read.
- The diagnosis records registered mailbox counts, provider/domain summaries,
  password-environment presence counts, local email root candidate counts,
  Apple Mail/Maildir/Thunderbird format coverage, and `collection_readiness`
  with `can_attempt_imap_collect`, `can_attempt_local_scan`, and
  `can_claim_real_validation=false`.
- `--diagnose-out <file>` writes the same JSON for FinClaw product preflight
  screens and runbooks.
- Fixture validation covers both a ready IMAP configuration and a local
  Maildir candidate root, while asserting that secrets, password environment
  variable names, full email addresses, local paths, long numeric account
  fragments, and body text are not emitted.

For `xueqiu-investor-activity`:

- `xueqiu_activity.py diagnose` now emits
  `collectorx.xueqiu_activity_preflight.v1` and can optionally write a standard
  `xueqiu_preflight_diagnosis_only` data-quality gap package.
- Diagnosis reports whether authorized exports, HAR files, ZIP packages, saved
  pages, copied browser-history files, or copy-required browser profile
  candidates can be attempted.
- Diagnosis does not collect activity events, parse HAR response bodies into
  events, emit browser-history URLs or titles, emit local paths, emit phone
  numbers, or claim broker trade collection.
- Package and SoulMirror bridge output now avoid local path emission and use
  stable source identifiers plus counts.

For `china-wealth-assets`:

- `source_preflight` records supported and unsupported source surfaces before
  parsing, including unsupported-source gaps when the user provided a file that
  cannot become wealth evidence.
- `china_wealth_asset_fact_contract.v1` requires minimum platform, product or
  account identity, and numeric value/cashflow fields before an asset,
  holding, cash-management, or cashflow row can enter the China wealth Lake.
- Rows missing the minimum field contract produce
  `china_wealth_field_contract_rejected_*` data-quality gap events instead of
  asset facts.
- Fixture validation proves a placeholder holding row is excluded from asset
  facts while usable asset and cashflow rows still enter Wiki evidence.

This improves product gating before collection and fact creation, but it does
...
- Added a checked-in Obsidian-style fixture under
  `examples/fixtures/notes-investment-e2e/` with a trade review, trade
  checklist/rules note, valuation-assumption note, and life-note decoy.
- `notes-collector` now has a repeatable fixture test proving the generic
  notes package emits four preview-only note events, keeps
  `can_claim_investment_notes=false`, and requires the `investment-notes` lens
  before Investor Wiki routing.
- The downstream `investment-notes` lens package is exercised against the
  generated notes Lake: it keeps three investment notes, excludes the life
  decoy, records preview-only boundary proof, and validates
  `investor_wiki_evidence.v1.json`.
- Package validation now covers both the generic `notes` package and the
  downstream `investment-notes` evidence package for this fixed fixture.

This improves repeatable product gating, but it does not claim real Notion,
Youdao, Evernote, Windows, Linux, or broad private-vault validation.

The prior completed wave adds stable offline fixture E2E gates for P0
...
- The shared CollectorX package validator now checks FinClaw readiness
  semantics, not only file structure: usable/gap counts, dedicated business
  counts, data-quality gap routing, gap-only business/Wiki gate failures,
  social-activity direct-Wiki blocking, and Investor Wiki evidence
  raw/gap/usable count alignment.
- `filesystem-collector` is upgraded to `0.3.3`; metadata-only packages now
  split usable/filesystem/file/gap counts, route no-metadata and filtered-all
  gaps to `collectorx.data_quality.collection_gaps`, expose
  filesystem/data-quality/research-documents-lens readiness gates, and keep
  `can_feed_investor_wiki_directly=false`.
- `xueqiu-watchlist` is upgraded to `0.3.4` and
  `xueqiu-investor-activity` to `0.3.7`; both Investor Wiki evidence packages
  now expose `generated_from.event_count`, `raw_event_count`, and
  `gap_event_count`, while gap-only packages produce no Wiki facts.
- `feishu` is upgraded to `0.2.1`; missing-input and no-readable packages now
  emit validator-safe data-quality gaps, and manifests expose usable/feishu/gap
  counts plus Feishu/data-quality/downstream-lens readiness gates.
- `wechat-favorites` is upgraded to `0.1.6`; missing, no-readable, and
  filtered-all runs now emit validator-safe data-quality gaps instead of empty
  packages, with usable/wechat-favorite/favorite/gap counts and
  WeChat-favorites/data-quality/article-lens readiness gates.

The prior completed wave: `social-activity` and `social-investment-influence` now expose explicit
social-activity/social-influence/data-quality/Wiki readiness gates:

- Upgraded `social-activity` to `0.2.9` and `investor-source-collectors` to
  `0.1.28`.
- `social-activity` manifests now separate `usable_event_count`,
  `social_activity_event_count`, and `gap_event_count`.
- `collection_readiness.can_enter_social_activity_lake` gates retained
  Weibo/Bilibili/Xiaohongshu activity traces, `can_enter_data_quality_lake`
  gates no-input/filtered-all gap packages, and
  `can_feed_social_investment_lens` blocks pure collection gaps from entering
  the downstream weak-evidence lens.
- The generic social collector still sets
  `can_feed_investor_wiki_directly=false`; the lens is required before Wiki
  evidence.
- `social-investment-influence` manifests now add
  `social_influence_event_count` and
  `collection_readiness.can_enter_social_investment_influence_lake`.
- Lens filtered-all gap packages route only to
  `collectorx.data_quality.collection_gaps`; retained lens events can feed
  `investor_wiki_evidence.v1.json`, but remain weak evidence that requires
  trade/research corroboration.

The prior completed wave: `pro-terminal-usage` now exposes explicit workflow/data-quality/Wiki readiness
gates and routes collection gaps to the data-quality Lake:

- Upgraded `pro-terminal-usage` to `0.2.9`.
- Manifests now separate `usable_event_count`, `workflow_event_count`, and
  `gap_event_count`.
- `collection_readiness.can_enter_pro_terminal_usage_lake` gates retained Wind,
  Choice, iFinD, and Bloomberg workflow metadata,
  `can_enter_data_quality_lake` gates no-input/filtered-all gap packages, and
  `can_feed_investor_wiki_evidence` blocks collection gaps from becoming
  investor Wiki facts.
...
The prior completed wave: `hk-us-brokerage` now exposes explicit strong-fact/data-quality/Wiki readiness
gates and routes collection gaps to the data-quality Lake:
...
- Upgraded `email-collector` to `0.5.8` and `investor-source-collectors` to
  `0.1.27`.
- Email manifests now separate `can_enter_email_lake`,
  `can_enter_data_quality_lake`, and `can_feed_email_research_lens` in
  `collection_readiness`.
- Email `mailbox_boundary_proof` now carries
  `can_feed_email_research_lens`, so FinClaw can block the lens on pure gap
  packages.
...
The prior completed wave: `research-documents` now emits validator-safe research-document collection gap
packages and separates usable research facts from data-quality gaps:

- Upgraded `investor-source-collectors` to `0.1.26`.
- Fully filtered research-document authorization scope runs now emit
  `research_documents_scope_policy_filtered_all` gap events with readiness
  `scope_policy_filtered_all`.
- Research-document gap events are `kind=profile`,
  `data.profile_type=investor_lens_collection_gap`, and route to
...
- Manifests now separate `event_count`, `usable_event_count`,
  ...
  exposes `can_enter_investor_source_lake`, `can_enter_data_quality_lake`, and
  `can_feed_investor_wiki_evidence`.
- Generated `investor_wiki_evidence.v1.json` counts only non-gap retained
  research-document events; pure gap packages report
  `generated_from.event_count=0`, `raw_event_count=1`, and
  `gap_event_count=1`.
- Fixture and CLI validation now run the shared CollectorX package validator
  for normal research-document packages and filtered-all gap packages.

The prior completed wave: `wechat` now routes generic communication collection gaps to data quality and
...
- When user-authorized Weibo, Bilibili, or Xiaohongshu activity records are
  fully excluded by platform, action, source-app, domain, creator, topic, or
  keyword authorization filters, the Lake file contains one profile gap event
  instead of an empty package.
- The filtered-all gap reason is `social_activity_scope_policy_filtered_all`;
...
- Missing or unusable authorized inputs emit
  `social_activity_authorized_input_missing` or
  `social_activity_records_empty` with the same validator-safe event shape.
- Manifest `social_activity_event_count=0` and `gap_event_count=1` keep package
  observability separate from real follow, like, favorite, watch, comment,
  share, browser-history visit, creator, engagement, social-topic, or weak
  influence evidence.
- Gap events carry candidate/filter counts and reason summaries but no raw
  local input path, credential, cookie, token, full creator profile, full
  content body, unrelated browser-history row, platform-wide scrape, investment
  influence fact, or investment conclusion.
- The generic collector still does not write Investor Wiki evidence directly;
  `social-investment-influence` remains the downstream weak-evidence lens.
- Fixture tests now run the shared CollectorX package validator for normal,
  no-input, and filtered-all packages.

The prior completed wave: `pro-terminal-usage` now emits validator-safe filtered-all and no-input gap
packages:
...
- If every candidate research document record is filtered by policy, readiness
  reports `scope_policy_filtered_all` and the package emits one
  `research_documents_scope_policy_filtered_all` data-quality gap event while
  the Investor Wiki fact evidence remains empty.
...
- Meeting artifact missing-input, empty-record, and filtered-all runs now emit
  validator-safe `kind=profile` gap events under
  `lake/meeting-artifacts/events.jsonl`.
- DingTalk/WeCom collaboration missing-input, empty-record, and filtered-all
  runs now emit validator-safe `kind=profile` gap events under
  `lake/dingtalk/events.jsonl` or `lake/wecom/events.jsonl`.
- Meeting manifests separate `event_count`,
  `meeting_artifact_event_count`, and `gap_event_count`; collaboration
  manifests separate `event_count`, `collaboration_event_count`, and
  `gap_event_count`.
- Gap events route to `collectorx.data_quality.collection_gaps` and are not
  promoted as meeting, collaboration, or investor Wiki facts.
...
The prior completed wave: `ticktick_events.py` and `calendar-collector` now
emit traceable task/calendar collection gap events:

- TickTick/Dida offline helper filtered-all and no-input runs now emit
  `kind=profile` gap events under `exports/ticktick/events.jsonl` while keeping
  the SoulMirror daemon as the only owner of `lake/ticktick/events.jsonl`.
- Calendar missing-input and filtered-all runs now emit validator-safe
  `kind=profile` gap events under `lake/calendar/events.jsonl`.
- TickTick manifests separate `event_count`, `task_event_count`, and
  `gap_event_count`; calendar manifests separate `event_count`,
  `calendar_event_count`, and `gap_event_count`.
- Gap events route to `collectorx.data_quality.collection_gaps` and are not
  promoted as planning facts or investor Wiki evidence.
...
- Missing user-selected note input now produces one `kind=profile` event with
  `data.profile_type=notes_collection_gap` and
  `status=needs_authorized_notes_input`.
- Source-policy filtered-all runs now produce one `kind=profile` event with
  `gap=notes_source_policy_filtered_all`, while `note_event_count=0` and
  `can_enter_finclaw=false`.
- Manifest separates `event_count`, `note_event_count`, and `gap_event_count`
  so FinClaw can ingest the package without promoting a collection gap as an
  investment note.
- Gap events route to `collectorx.data_quality.collection_gaps`; the
  `investment-notes` lens remains responsible for investor Wiki facts.
...
- As of version `0.2.8`, if all candidate records are outside the authorized
  ...
  one validator-safe `social_activity_scope_policy_filtered_all` profile gap
