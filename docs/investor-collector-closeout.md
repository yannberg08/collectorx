...
最终交接清单见 `docs/final-handoff-checklist.md`。如果只需要接手项目、配置
FinClaw 调用或安排下一轮真实验证，先读最终交接清单，再按本文追溯验收口径。
...
这次纠偏后的项目口径是：30 个 catalog 条目是投资分身的数据源地图，不是 30 个
已经投产的采集器。继续把 P0/P1/P2 全部同时深挖，会让项目变成长期研发工程，而不
是一个可交付的 CollectorX 版本。

因此当前收口目标改为：

1. 不再新增数据源，也不把 beta 条目包装成生产能力。
2. 只暴露 `eastmoney-portfolio` 为受保护的生产候选。
3. 只把 `ths-portfolio` 和 `qq` 暴露为邀请制 deep beta。
4. 其他条目保持 `baseline+audit`，通过真实验证证据账本逐个提升，而不是靠文档
   或目录存在提升。
5. 后续每次 readiness 提升都必须先有
   `docs/real-validation-evidence-ledger.md` 规定的真实证据，再跑
   `readiness-review` 和 `readiness-change-audit`。
