...
| investor-source-collectors | 8+ | 投资分身分类、路由和 lens 证据包工具；research-documents 已补逐输入审计、显式内容抽取、`--include-image-ocr` 本地截图 OCR 审计、研究语料边界证明和研究文档面谱，investment-notes 与 task-calendar-investor 已补投资面谱，不替代通用/垂直源采集器 |
