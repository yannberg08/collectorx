...
description: 专业金融终端使用痕迹采集器。采集用户授权的 Wind、Choice、同花顺 iFinD、Bloomberg 等终端工作台、自选、搜索、下载、模型模板、因子关注等个人工作流元数据，支持授权 ZIP 包，输出 CollectorX 事件、终端/活动/字段覆盖、工作流主题面谱、工作流可用面、来源审计、许可边界 manifest 和 FinClaw 投资分身证据包；不复制厂商数据库内容，不采凭据。
---
...
边界，并把用户工作流归类到研究主题面谱。采集器记录用户“怎么研究、关注什么、常用什么工具”，
不是公共金融数据采集器。
