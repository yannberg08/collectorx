...
description: 微博、B站、小红书等社交平台用户活动通用采集器。采集用户授权的关注、点赞、收藏、观看历史、评论、分享和浏览器历史副本等个人活动，支持授权 ZIP 包和平台/动作/来源应用/域名/创作者/主题/关键词授权范围过滤，输出 CollectorX 事件、validator-safe filtered-all/no-input data-quality gap package、gap/usable/social-activity 计数、social-activity/data-quality/social-investment-lens readiness gates、弱证据策略、平台/动作/字段覆盖、社交影响主题面谱、浏览器历史来源审计、影响源可用面、社交活动边界证明、来源审计和内容策略 manifest；不抓平台全站内容，不直接形成投资结论。
---
...
- `manifest.source_audit.social_activity_scope_policy`：记录平台、动作、来源应用、域名、创作者、主题和关键词授权范围；若候选记录全部被策略排除，readiness 为 `scope_policy_filtered_all`，写入 1 条 profile gap 事件，不写入假社交活动事件。
- `manifest.usable_event_count`、`manifest.social_activity_event_count` 与 `manifest.gap_event_count`：区分真实社交活动弱证据和采集缺口。
- `manifest.collection_readiness.can_enter_social_activity_lake`、`can_enter_data_quality_lake`、`can_feed_social_investment_lens` 与 `can_feed_investor_wiki_directly`：正常社交活动只进入 `internal.social.activity` 并可喂下游 lens；filtered-all/no-input data-quality gap package 的社交活动湖和 lens gate 为 false、数据质量湖 gate 为 true、直接 Wiki gate 始终为 false。
...
授权范围策略用于把用户明确授权外的平台、创作者、域名和主题排除在 Lake 之前；filtered-all/no-input 包只记录 collection gap，路由到 `collectorx.data_quality.collection_gaps`，不会成为社交活动事实或投资影响源事实。它不会把弱证据升级为投资结论。
