...
description: 微博、B站、小红书等社交平台用户活动通用采集器。采集用户授权的关注、点赞、收藏、观看历史、评论、分享和浏览器历史副本等个人活动，支持授权 ZIP 包和平台/动作/来源应用/域名/创作者/主题/关键词授权范围过滤，输出 CollectorX 事件、validator-safe filtered-all/no-input gap package、弱证据策略、平台/动作/字段覆盖、社交影响主题面谱、浏览器历史来源审计、影响源可用面、社交活动边界证明、来源审计和内容策略 manifest；不抓平台全站内容，不直接形成投资结论。
---
...
- `manifest.social_activity_event_count` 与 `manifest.gap_event_count`：区分真实社交活动弱证据和采集缺口；filtered-all/no-input gap package 的 `social_activity_event_count=0`、`gap_event_count=1`、`collection_readiness.can_enter_finclaw=false`。
...
授权范围策略用于把用户明确授权外的平台、创作者、域名和主题排除在 Lake 之前；filtered-all/no-input 包只记录 collection gap，不会成为社交活动事实或投资影响源事实。它不会把弱证据升级为投资结论。
