...
description: 港美股券商强交易采集器。采集用户授权的富途、老虎、盈透等券商只读导出/ZIP 包中的资产、持仓、成交、委托、资金流水、分红、换汇，输出 CollectorX 事件、券商/交易表/字段覆盖、账户边界、币种市场、费用税费保证金、资金流活动、收益回报、订单执行、授权范围过滤审计、强交易可用面、资产数值汇总、来源审计 manifest 和 FinClaw 投资分身证据包；不读取密码，不下单，不撤单。
---
...
- `manifest.cashflow_activity_summary`、`manifest.income_return_summary` 与 `manifest.order_execution_summary`：汇总入金/出金、分红税前税后、换汇金额、净现金流、分红标的、P/L、费用拖累、税费拖累、委托状态、买卖方向、订单类型、有效期、成交编号、交收日和填单比例。
...
支持的过滤参数包括 `--allow/--deny-broker`、`--allow/--deny-account`、
`--allow/--deny-subtype`、`--allow/--deny-symbol`、`--allow/--deny-market`、
`--allow/--deny-currency` 和 `--allow/--deny-keyword`。如果授权范围把所有
候选记录排除，采集器会写出 `scope_policy_filtered_all`，不会伪造缺输入 gap。
