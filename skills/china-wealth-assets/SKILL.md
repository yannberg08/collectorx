...
`china_wealth_scope_policy_filtered_all` profile gap 事件，路由到
`collectorx.data_quality.collection_gaps`，不会伪造空成功或业务资产记录。
`manifest.asset_boundary_proof.authorization_scope_boundary` 会给 FinClaw 检查资产授权边界。
manifest 会拆分 `usable_event_count`、`asset_event_count` 和 `gap_event_count`，
并用 `collection_readiness.can_enter_china_wealth_lake`、
