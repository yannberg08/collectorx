...
- `manifest.source_audit.financial_news_scope_policy`：记录平台、动作、来源应用、域名、主题和关键词 allow/deny 授权过滤；同时记录候选记录数、过滤记录数、过滤原因和 `scope_policy_filtered_all` 状态。
...
`scope_policy_filtered_all`，不会伪造 gap 事件。
...
- 已验证：JSON/CSV/HTML/Markdown/TXT/ZIP 授权导出、Chromium 浏览器历史库、Safari `History.db` 直接文件、ZIP 包内 Safari `History.db`、平台/动作/主题归一、搜索/订阅/提醒/停留/访问行为字段、平台/动作/来源应用/域名/主题/关键词授权范围过滤、字段覆盖、浏览器来源计数、逐输入来源审计、跳过原因、使用边界证明、内容/证据边界、递归敏感字段过滤。
