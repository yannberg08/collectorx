...
description: 财经资讯使用痕迹垂直采集器。采集用户授权的财联社、华尔街见闻、格隆汇等财经 App/网页的阅读、收藏、订阅、搜索、自选提醒等个人动作，支持文件/目录/ZIP/浏览器历史副本，输出 CollectorX 事件、平台/动作/主题覆盖、字段覆盖、来源审计、内容策略 manifest 和 FinClaw 投资分身证据包；不采公共新闻全文库。
---
...
- `manifest.usage_boundary_proof`：汇总授权输入、平台/动作/主题覆盖、浏览器历史域名过滤、ZIP 安全、内容指针和完整性声明；明确不声明完整使用历史、完整账号边界、公共新闻全文爬取、公共文章镜像或平台全站数据。
...
- 已验证：JSON/CSV/HTML/Markdown/TXT/ZIP 授权导出、Chromium 浏览器历史库、平台/动作/主题归一、字段覆盖、逐输入来源审计、跳过原因、使用边界证明、内容/证据边界、递归敏感字段过滤。
