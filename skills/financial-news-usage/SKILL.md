...
description: 财经资讯使用痕迹垂直采集器。采集用户授权的财联社、华尔街见闻、格隆汇等财经 App/网页的阅读、收藏、订阅、搜索、自选提醒等个人动作，支持文件/目录/ZIP/直接或 ZIP 内浏览器历史副本，输出 CollectorX 事件、平台/动作/主题覆盖、字段覆盖、行为强度、授权范围过滤审计、来源审计、业务/数据质量/Wiki 入湖门禁、内容策略 manifest 和 FinClaw 投资分身证据包；不采公共新闻全文库。
---
...
- `manifest.source_audit.financial_news_scope_policy`：记录平台、动作、来源应用、域名、主题和关键词 allow/deny 授权过滤；同时记录候选记录数、过滤记录数、过滤原因和 `scope_policy_filtered_all` 状态；授权过滤排空时会写出可验证的 profile gap 事件。
- `manifest.usable_event_count`、`manifest.usage_event_count` 和 `manifest.gap_event_count`：区分真实个人资讯使用痕迹和数据质量缺口。
- `manifest.collection_readiness.can_enter_financial_news_usage_lake`、`can_enter_data_quality_lake` 与 `can_feed_investor_wiki_evidence`：分别控制业务 Lake、数据质量 Lake 和投资分身 Wiki evidence；no-input/filtered-all gap 只进入 `collectorx.data_quality.collection_gaps`。
...
支持的过滤参数包括 `--allow/--deny-platform`、`--allow/--deny-action`、
`--allow/--deny-source-app`、`--allow/--deny-domain`、`--allow/--deny-topic`
和 `--allow/--deny-keyword`。如果授权范围把所有候选记录排除，采集器会写出
`financial_news_scope_policy_filtered_all` profile gap 事件；该事件只记录计数、过滤原因和边界声明，
不写入公共新闻正文、不写入本地输入路径，也不会作为可用个人资讯使用事实进入 Wiki。
...
- 已验证：JSON/CSV/HTML/Markdown/TXT/ZIP 授权导出、Chromium 浏览器历史库、Safari `History.db` 直接文件、ZIP 包内 Safari `History.db`、平台/动作/主题归一、搜索/订阅/提醒/停留/访问行为字段、平台/动作/来源应用/域名/主题/关键词授权范围过滤、filtered-all/no-input data-quality gap 包、业务/数据质量/Wiki 入湖门禁、字段覆盖、浏览器来源计数、逐输入来源审计、跳过原因、使用边界证明、内容/证据边界、递归敏感字段过滤。
