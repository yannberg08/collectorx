...
description: 通用会议产物采集器。采集用户授权的会议纪要、逐字稿、字幕、参会人和附件指针，支持本地文件/ZIP/平台导出，输出 CollectorX note 事件、平台覆盖 manifest 和 filtered-all/no-input gap package；用于 feeding meeting-minutes lens，不直接判断投资含义。
---
...
- 授权输入缺失、可读记录为空或授权范围过滤为空时，输出 `profile` gap 事件，
  并在 manifest 中区分 `meeting_artifact_event_count` 和 `gap_event_count`；
...
- 已验证：来源平台、参会人和关键词授权范围过滤，以及所有候选会议被过滤时的 validator-safe `scope_policy_filtered_all` gap package。
