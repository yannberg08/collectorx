...
### FinClaw/CollectorX 预检

面向 FinClaw 产品入口时，先跑只读诊断，不要直接采消息：

```bash
python3 <本 skill 目录>/scripts/wechat_query.py --diagnose --diagnose-out <diagnose.json>
```

诊断输出 `collectorx.wechat_preflight.v1` JSON，只记录平台、依赖、SIP、本地库/密钥
readiness 和下一步动作；它不读取消息正文、联系人、原始数据库页，不输出密钥或本地路径。
只有 `collection_readiness.can_attempt_collect=true` 后，才进入
`--collect --out-dir <out-dir>` 标准包采集；真实消息包通过 validator 后，才能喂给
...
# 给 CollectorX / FinClaw 使用：写标准采集包
python3 scripts/wechat_query.py --collect --days 3 --active-group-days 3 --out-dir /tmp/wechat-package
...
# 输出 CollectorX 标准包，供 wechat-investment-dialogue lens 继续筛投资对话
python3 wechat_query.py --collect --days 7 --out-dir /tmp/wechat-package
