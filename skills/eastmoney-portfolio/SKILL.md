...
不得输出登录凭据、会话凭据、签名、设备指纹、原始通信负载、浏览器存储体。用户授权后的本地 Lake 需要保留完整业务数据，包括账户标识、股东账号、委托号、成交号、金额、数量和价格；这些字段属于投资分身可用的个人交易事实。
...
- 交易流水：成交日期/时间、证券代码、名称、买卖方向、成交价格、数量、金额、手续费、印花税、过户费、成交号、委托号。
- 委托与撤单：委托日期/时间、方向、委托价、委托数量、成交数量、撤单数量、委托状态、委托号。
...
- 交易密码、解锁密码、下单确认信息、会话 token、签名、设备指纹、原始网络包。
...
- 授权采集的个人交易字段必须保留原值进入本地 Lake；`privacy.sensitive=true` 和 `local_only=true` 负责标记敏感性，不能用脱敏破坏投资分身分析所需事实。
...
- 优先使用 macOS ApplicationServices Accessibility 按坐标读取账户面板和交易表格；如果系统不返回控件树，才临时截取东方财富窗口做本机 OCR，只用于识别“已锁定/未登录/已登录”等状态。
- 先从交易页可访问文本提取可见资产字段；在账户未锁定时，依次尝试用 Accessibility 表格重建和剪贴板复制读取资产、持仓、成交、委托、资金表。
- 把复制出的表格直接解析为强交易事实事件。
- 采集完成后恢复系统剪贴板。
...
该入口仅用于历史兼容、测试、异常兜底。账号、股东号、委托号、成交号、金额、数量、价格、盈亏都会按原值写入本地 Lake，并用 `privacy.sensitive/local_only` 标记；不会上传，也不会写入 Git。
...
`--probe-export` 会输出平台探测报告，只包含平台、扫描根目录标签、找到的东方财富本机数据目录数量和能力状态。探测报告会对路径标签做保护；正式采集包会保留授权后的业务字段原值。不读取登录材料、签名、设备指纹或原始交易通信负载。
...
- `ready_for_investor_avatar`：资产、持仓、成交、委托、资金流水强表均已形成事件，或表格被确认可读且确为空。
...
- 如果交易页表格可读但为空，采集器会记录确认空表状态；如果不可读，才输出表格不可读缺口。
