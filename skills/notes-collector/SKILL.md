...
description: 采集笔记应用数据。当用户说"导出笔记"、"采集笔记"、"笔记备份"时使用此skill；支持按来源应用、路径/笔记本和标签限定用户授权范围。
---
...
# 只采用户明确授权的投资笔记范围，例如 Notion Export 下带 #投资 或 #规则 的笔记
python <SKILL_DIR>/scripts/notes_api.py import \
  --input ~/Downloads/notes-export \
  --source-app auto \
...
| `--allow-source-app` / `--deny-source-app` | 按来源应用收窄或排除授权范围，可重复或逗号分隔 |
| `--allow-path` / `--deny-path` | 按路径、笔记本、文件夹关键词收窄或排除授权范围，可重复或逗号分隔 |
| `--allow-tag` / `--deny-tag` | 按标签或正文中的 `#tag` 收窄或排除授权范围，可重复或逗号分隔 |
...
如用户只授权部分 vault、notebook、文件夹或标签，`import` 和 `obsidian`
命令可使用 `--allow-source-app`、`--allow-path`、`--allow-tag` 以及对应
`--deny-*` 参数。`manifest.source_audit.note_source_policy` 会记录策略、
候选笔记数量、被过滤数量和过滤原因；该策略只表达授权范围，不直接声明投资相关性。
...
扩展名覆盖、CSV/TSV 表格文件数、表格行数、Canvas 文件数、Canvas note 数、授权范围策略、跳过文件原因、ZIP 成员数量、跳过 ZIP 成员原因、来源引用和路径
